...
customers' identities, which is exactly the combination an enterprise
security team cares about. Isolating both (secrets that never sit in
plaintext config, and PII that never crosses into contract memory) turns
"trust us" into an architectural guarantee instead of a policy.
...
rather than guessed from docs/search. An earlier pass had a real bug here
...
- `stripe_body_is_not_json_but_paystack_body_is` is a concrete, runnable
  ...
  while Paystack's is. This is exactly why one path can complete live on
...
- `driver/setup-profile.ts` attempts to populate this test tenant's own
  ...
  `{{profile.verified_contacts.*}}` placeholders to resolve). See "Testing
  a dispute end-to-end" below for what this uncovered.
...
(`driver/setup-profile.ts`: `otpRequest` then `otpVerify` then
`submitUserInput`) surfaced two separate issues while working through it,
one self-inflicted, one real:

- Early attempts used a placeholder test address on the `example.com`
  domain, which is reserved for documentation and does not accept real
  mail. That produced a misleading `host/otp.verify: Email provider
  temporarily unavailable` error on every attempt, which was reported to
  T3N as a suspected platform outage before the actual cause (a
  non-deliverable test address, not a real outage) was identified.
  Switching to a real, deliverable email address made that specific error
  go away immediately.
- With a real address, `otpRequest` now succeeds cleanly (returns a
  `txHash`, no error), but no email is ever received (checked spam too),
  and an immediate `otpVerify` call against the same request, including
  with an added delay to rule out a timing race, fails with
  `host/otp.verify: OTP code provided but no OTP state found`. So the
  request appears to be accepted and committed, but does not create the
  server-side state needed for verification, and no email is actually
  dispatched. This looks like a real bug in the `otp-request` to
  `otp-verify` pipeline, reported to T3N along with a repro.
...
`tenant.contracts.register()`. Note: re-registering the same tail with a
new version allocates a new `contract_id` (869 to 917 going from v0.1.0 to
...
`submit-dispute-evidence` is **confirmed non-functional on Stripe, for a
platform-level reason, not a contract bug**. See "Known issue #2" below.
Confirmed reaching real placeholder resolution on Paystack (fails only on
a caller-profile gap, `PlaceholderUnknown: verified_contacts.phone.value`,
...
`5.3.0`; every version before it (`5.0.0` through `5.2.0`) validates the
exact same live manifest successfully. There is no safe client-side
workaround: the manifest's `signature` is a server-side hash over the
canonical payload minus `signature` itself, so patching in a synthetic
`rtmr1_allowlist` client-side to pass the shape check breaks signature
...
in the JS/TS SDK's client-side manifest validator, not in anything the
Rust contract does.

Independently corroborated: `z-tenant-flight` (T3N's own public reference
repo) has an open issue (#11) reporting the same root symptom, filed by
...
classic REST API, for example `/v1/disputes/:id` and much of the rest of
...
### Issue #3: `otp-request` accepts and commits, but never creates verifiable state

Live-tested against a real, deliverable email address (not the placeholder
domain mistake noted in "Testing a dispute end-to-end" above): `otpRequest`
returns success with a `txHash` and no error, but no email is ever
delivered, and an immediate `otpVerify` call against the same request
(tested with and without an added delay, to rule out a timing race) fails
with `host/otp.verify: OTP code provided but no OTP state found`.

**Impact:** on this testnet cluster, no profile field can currently be
verified end-to-end through the documented `otpRequest` then `otpVerify`
then `submitUserInput` flow, which blocks any contract relying on
`{{profile.verified_contacts.*}}` placeholders for a fresh tenant identity,
